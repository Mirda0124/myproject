 ...
 if (web=="DefaultNastav"){   
  ...
  function Butt_DiffOnUp() {
   ...
   connection.send(butt);
         ...
         function Butt_update_soft() {connection.send('u');}   
         function Butt_update_spiff() {connection.send('p');} 
       ...
       var hodnota;       
         ...
         var hodnota;
                   ...
                   function dialog4(){
                   document.getElementById('schema').style.background = '#60A917';
                   document.getElementById('nastaveni').style.background = '#808080';
                   document.getElementById('graf').style.background = '#808080';
          var dialog2= TINY.box;
  dialog2.show({url:'schema.html',boxid:'frameless2',width:820,height:573,fixed:false,maskid:'greymask',maskopacity:40,loading:0,left: '170px',top:'170px',mask:false,eschide:1,close_butt:1,closejs:function(){DntsendSchm()} })
    ...
    function dialog1(){
                  document.getElementById('schema').style.background = '#808080';    
                   document.getElementById('nastaveni').style.background = '#60A917';
                   document.getElementById('graf').style.background = '#808080';
    
                            
        var  kukiny = document.cookie.split(";"); // rozøezávám øetìzec kukin do pole
    for (i in kukiny){ // procházím pole kukin
       var jednaKukina = kukiny[i].split("="); // rozøezávám kukinu do dvouprvkového pole
        if (jednaKukina[0] == 'ESPSESSIONID') hodnota=jednaKukina[1]; // jestli¾e je to ta pravá kukina, vybírám její hodnotu z druhé polo¾ky pole za rovnítkem
                     }
                         
       if (hodnota=='1') {    
          var dialog1= TINY.box;
          dialog1.show({url:'m.NastaveniA.html',boxid:'frameless',width:405,height:680,fixed:false,maskid:'greymask',maskopacity:40,loading:0,left: '390px',top:'170px',mask:false,eschide:1,close_butt:1,closejs:function(){DntsendSolar()} })
             connection.send("d");
           }
           
        if (hodnota=='2') {    
          var dialog1= TINY.box;
          dialog1.show({url:'m.NastaveniU.html',boxid:'frameless',width:410,height:300,fixed:false,maskid:'greymask',maskopacity:40,loading:0,left: '390px',top:'170px',mask:false,eschide:1,close_butt:1,closejs:function(){DntsendSolar()} })
             connection.send("d");
           }  
           
         if (hodnota=='3') {    
          var dialog1= TINY.box;
          dialog1.show({url:'m.nastaveniP.html',boxid:'frameless',width:810,height:580,fixed:false,maskid:'greymask',maskopacity:40,loading:0,left: '170px',top:'170px',mask:false,eschide:1,close_butt:1,closejs:function(){DntsendSolar()} })
             connection.send("d");
           }            
    ...
    function graf() { 
                   document.getElementById('nastaveni').style.background = '#808080';
                  document.getElementById('schema').style.background = '#808080';
                   document.getElementById('graf').style.background = '#60A917'; 
    TINY.box.show({url:'m.Graf.html',boxid:'frameless1',width:1168,height:600,fixed:false,maskid:'greymask',maskopacity:40,loading:0,left: '-8px',top:'170px',mask:false,eschide:1,close_butt:1, })  
              }       
          ...
          var dialog2= TINY.box;
          dialog2.show({url:'NastavCidla.html',boxid:'frameless',width:410,height:250,fixed:false,maskid:'greymask',maskopacity:40,loading:0,left: '40px',top:'170px',mask:false,eschide:1,close_butt:1, })
