...
//WEB SOOOOOOOOOOOOOOOOOCKET
 ...
 // 'use strict';
  ...
  //  connection = new WebSocket('ws://'+location.hostname+':81/');
     connection = new WebSocket('ws://192.168.1.144:81/');
  ...
  //    function check(){    
//    if(!connection || connection.readyState == 3  ) {     setInterval(start(), 5000);}  }
...
// function close(){dialog.unload();}
  ...
  function odpojeno(){
  if(ztraceno==true){dialog_load3.unload();}
   if (ztraceno==false){
   ...
   connection.send(butt);
    ...
    for (i in kukiny){ // procházím pole kukin
       var jednaKukina = kukiny[i].split("="); // rozøezávám kukinu do dvouprvkového pole
        if (jednaKukina[0] == 'login') hodnota=jednaKukina[1]; // jestli¾e je to ta pravá kukina, vybírám její hodnotu z druhé polo¾ky pole za rovnítkem
                     }
...
var ctx = document.getElementById("Graf").getContext("2d");
 ...
 function logout(){ document.cookie ='login=0'; alert("odlhásit");window.location.replace("/login.html")}
